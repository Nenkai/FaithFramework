...
public unsafe class SaveHooks : HookGroupBase
{
    ...
    public override void SetupHooks()
    {
        ...
        Project.Scans.AddScanHook(nameof(SerializeSave), 
            (result, hooks) => SerializeSaveHook = hooks.CreateHook<SerializeSave>(SerializeSaveDetour, result).Activate());
        Project.Scans.AddScanHook(nameof(SerializeSystemSave), 
            (result, hooks) => SerializeSystemSaveHook = hooks.CreateHook<SerializeSystemSave>(SerializeSystemSaveDetour, result).Activate());

        //DeserializeSaveHook = _scans.CreateHook<DeserializeSave>(DeserializeSaveDetour, _modConfig.ModId);

        Project.Scans.AddScanHook(nameof(CreateXmlSerializer),
            (result, hooks) => CreateXmlSerializerHook = hooks.CreateHook<CreateXmlSerializer>(CreateXmlSerializerDetour, result).Activate());
    }
    ...
    public nint SerializeSaveDetour(nint a1, nint a2, nint saveEntity, string fileName, bool saveAsXml)
    {
        if (_configuration.SerializeSavesAsXml)
        {
            ...
            return SerializeSaveHook?.OriginalFunction(a1, a2, saveEntity, fileName, true) ?? 0;
        }
        else
            return SerializeSaveHook?.OriginalFunction(a1, a2, saveEntity, fileName, false) ?? 0;
    }
    ...
    public nint SerializeSystemSaveDetour(nint a1, nint saveEntity,  bool saveAsXml)
    {
        if (_configuration.SerializeSavesAsXml)
        {
            ...
            return SerializeSystemSaveHook?.OriginalFunction(a1, saveEntity, true) ?? 0;
        }
        else
            return SerializeSystemSaveHook?.OriginalFunction(a1, saveEntity, false) ?? 0;
    }
    ...
    */
    ...
    public nint CreateXmlSerializerDetour(nint a1, string fileName, uint version)
    {
        if (fileName == "config.xml")
            return CreateXmlSerializerHook?.OriginalFunction(a1, fileName, version) ?? 0;

        return CreateXmlSerializerHook?.OriginalFunction(a1, Path.GetFileNameWithoutExtension(fileName), version) ?? 0;
    }
