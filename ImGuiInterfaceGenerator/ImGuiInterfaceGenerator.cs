...
public class ImGuiInterfaceGenerator
{
    ...
    public string FinishInterface(string namespaceDeclaration)
    {
        ...
        FileScopedNamespaceDeclarationSyntax namespaceDecl = SF.FileScopedNamespaceDeclaration(SF.ParseName(namespaceDeclaration))
            ...
            if (mem is NamespaceDeclarationSyntax namespace_)
            {
                ...
                foreach (MemberDeclarationSyntax methodClassMember in methodsClass.Members)
                {
                    ...
                    for (int i = 0; i < newParamList.Count; i++)
                    {
                        ...
                        else if (typeName.StartsWith("ImStructVectorWrapper")) // For weird cases like 'ImGuiTextFilter_ImGuiTextRange_split'.
                        {
                            // Hacky. but works
                            ...
                            var body = SF.Block(SF.List([
                                SF.LocalDeclarationStatement(
                                    ...
                                    .WithVariables(
                                        SF.SingletonSeparatedList<VariableDeclaratorSyntax>(
                                            SF.VariableDeclarator(
                                                SF.Identifier("retPtr"))
                                            .WithInitializer(SF.EqualsValueClause(callExpression))))),
                                SF.ParseStatement("if (retPtr is null)"),
                                SF.ParseStatement("    return null!;"),
                                SF.ParseStatement($"var retImpl = new {returnIdentifier.Identifier.ValueText.Substring(1)}(retPtr);"),
                                SF.ParseStatement("return retImpl;")
                            ]));
                            newMethod = newMethod.WithBody(body);
                        }
                        else
        ...
        string nativeStructName, string fieldName, ref TypeSyntax newType)
    {
        ...
        else if (newType.IsKind(SyntaxKind.GenericName))
        {
            ...
            string name = genericName.TypeArgumentList.Arguments[0].ToString();
            if (newTypeName.Contains("RangeAccessor"))
            {
                ...
                bodyExpression = SyntaxTreeUtils.CreateRangeAccessorExpression(name, nativeStructName, "NativePointer", fieldName, length);
            }
            else if (newTypeName.Contains("RangeStructAccessor"))
            {
                ...
                bodyExpression = SyntaxTreeUtils.CreateRangeStructAccessorExpression(name, name.Substring(1), nativeStructName, "NativePointer", fieldName, length);
            }
            else if (newTypeName.Contains("ImStructPtrVectorPtrWrapper"))
            {
                bodyExpression = SyntaxTreeUtils.CreateImStructPtrVectorPtrAccess(name, name.Substring(1), nativeStructName, "NativePointer", fieldName);
            }
            else if (newTypeName.Contains("ImStructPtrVectorWrapper"))
            {
                bodyExpression = SyntaxTreeUtils.CreateImStructPtrVectorAccess(name, name.Substring(1), nativeStructName, "NativePointer", fieldName);
            }
            else if (newTypeName.Contains("ImStructVectorWrapper"))
            {
                bodyExpression = SyntaxTreeUtils.CreateImPtrVectorAccess(name, name.Substring(1), nativeStructName, "NativePointer", fieldName);
            }
            else if (newTypeName.Contains("ImVector"))
            {
                bodyExpression = SyntaxTreeUtils.CreatePointerAccessExpressionForGetter(nativeStructName, "NativePointer", fieldName);
            }
        }
    ...
    public string FinishImpl(string namespaceDeclaration, string interfacesNamespace)
    {
        ...
        CompilationUnitSyntax root = SF.CompilationUnit()
            .WithUsings(SF.List(new List<UsingDirectiveSyntax>()
            {
                ...
                SF.UsingDirective(SF.ParseName("System.Runtime.InteropServices")),
                SF.UsingDirective(SF.ParseName(interfacesNamespace)),
                SF.UsingDirective(SF.ParseName("FF16Framework.Native.ImGui")),
            }))
    ...
    public void ExtractBindingsFromSyntaxTree(SyntaxTree syntaxTree, bool isBackend)
    {
        ...
        foreach (MemberDeclarationSyntax mem in bindingsRoot.Members)
        {
            if (mem is NamespaceDeclarationSyntax namespace_)
            {
                ...
                {
                    if (member.IsKind(SyntaxKind.ClassDeclaration) && ((ClassDeclarationSyntax)member).Identifier.Text == "Methods" ||
                        member.IsKind(SyntaxKind.EnumDeclaration)) // No point redeclaring enums (we did it in the interface)
                        continue;
    ...
    public string FinishBindings(string namespaceDeclaration)
    {
        ...
        CompilationUnitSyntax root = SF.CompilationUnit()
            .WithUsings(SF.List(new List<UsingDirectiveSyntax>()
            {
                ...
                SF.UsingDirective(SF.ParseName("System.Runtime.InteropServices")),
                SF.UsingDirective(SF.ParseName("System.Numerics")),
                SF.UsingDirective(SF.ParseName("FF16Framework.Interfaces.ImGui")),
            }))
            .WithMembers(new SyntaxList<MemberDeclarationSyntax>(namespaceDecl))
            .NormalizeWhitespace();
        ...
        out SyntaxTokenList modifiers, bool doNotRemapStrings = false, bool pointersToNint = false, bool isArrayElement = false)
    {
                ...
                if (type is PointerTypeSyntax pointerType)
                {
                    ...
                    else
                    {
                        if (typeFixKind == ImGuiStructTypeKind.ForInterface)
                            return SF.ParseTypeName($"ImStructPtrVectorPtrWrapper<I{GetCorrectedType(vectorElemName)}>");
        ...
        if (nativeTypeName is not null)
        {
            ...
            else
            {
                ...
                var translated = TranslateTypeNameIntoCSharpType(currentParamType, nativeTypeName, doNotRemapStrings);
        ...
        else
        {
            ...
            switch (pointerTypeDesc.InnerType)
            {
                case UserDescriptionMetadata userDesc:
                    {
                        ...
                        {
                            if (typeFixKind == ImGuiStructTypeKind.ForInterface)
                                return SF.ParseTypeName($"ImStructVectorWrapper<I{userDesc.Name ?? userDesc.Name}>");
                            else if (typeFixKind == ImGuiStructTypeKind.ForNativeStruct)
                ...
                case PointerDescriptionMetadata pointerDesc:
                    ...
                    string vectorName = typeName.EndsWith("Ptr") ? GetStructPtrVectorNameForContext(typeFixKind) : "ImStructVectorWrapper";
    ...
    private static string GetStructPtrVectorNameForContext(ImGuiStructTypeKind kind)
    {
        if (kind == ImGuiStructTypeKind.ForNativeStruct)
            ...
            return "ImStructPtrVectorWrapper";
    ...
    private static TypeSyntax? TranslateTypeNameIntoCSharpType(string currentParamType, string? nativeTypeName, bool doNotRemapStrings)
    {
